...
export function City({ name, number, x, y, labelPosition = "top" }) {
  ...
  return (
    ...
    >
      <span className="text-sm font-bold text-zinc-800 dark:text-zinc-100 ">{number}</span>
      {name && (
        <div
          className={`absolute whitespace-nowrap text-xs font-bold text-white pointer-events-none ${getLabelStyle(labelPosition)}`}
          style={{
            textShadow:

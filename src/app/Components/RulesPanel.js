...
const PANELS = [
  ...
  <div key={2} className="flex flex-col items-center gap-4">
    <p className="text-zinc-800 dark:text-zinc-200 font-semibold">
      ...
      yellows for a 2 yellow route. Wild-rainbow cards can replace any color
      (for example one green and one rainbow for a 2 green route). Gray routes
      can be claimed by spending any colored cards, but the cards should be the
      same color such as 2 greens or 2 reds.
    </p>
    <Image
      src="/Route.jpg"
  ...
  <div key={4} className="flex flex-col items-center gap-4">
    <p className="text-zinc-800 dark:text-zinc-200 font-semibold">
      ...
      cannot place your tiles in both routes — the other route may be used by
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-100 dark:bg-zinc-900 font-sans p-8">
      <div className="relative bg-white dark:bg-zinc-800 p-12 rounded-[40px] shadow-2xl border border-zinc-200 dark:border-zinc-700 flex flex-col items-center max-w-md w-full text-center min-h-64">
        <button
          onClick={onFinish}
          className="absolute top-4 left-4 w-8 h-8 cursor-pointer flex items-center justify-center rounded-full bg-zinc-200 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-300 font-bold text-sm transition-colors"
        >
          ✕
        </button>
          ...
          {PANELS.map((_, i) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-colors ${
                i === page ? "bg-zinc-800" : "bg-zinc-300 dark:bg-zinc-600"
              }`}
            />
          ))}
        </div>
        ...
        <div className="mt-10 flex gap-4">
          ...
          <button
            onClick={isLast ? onFinish : () => setPage((p) => p + 1)}
            className="px-8 py-3 cursor-pointer rounded-2xl bg-zinc-800 text-white font-black text-sm uppercase tracking-widest hover:scale-105 transition-transform shadow-lg"
          >
            {isLast ? "Finish" : "Next"}
          </button>
        </div>

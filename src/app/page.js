...
export default function Home() {
  ...
  const isExtraManualTurn = !isAiTurn && currentPlayerIndex > 0;
  ...
  const getUsefulRoutesForTicket = useCallback(
    (cityA, cityB, claimedRoutes, aiIdentifier) => {
      const adj = new Map();
  ...
  const executeAiAction = useCallback(
    ...
    [gameState, aiDifficulty, updateGameState, incrementTurn],
  );
...
4. If all tickets done, draw_tickets.`
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-zinc-100 dark:bg-zinc-900 font-sans p-8">
      ...
      <PlayerBoard
        playerHand={currentPlayerData?.hand ?? EMPTY_HAND}
